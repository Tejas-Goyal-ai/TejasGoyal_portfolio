# Hi there, I'm Tejas Goyal 👋

### Generative AI Developer | AI Automation Builder 

I build practical AI applications that solve real-world problems using Generative AI, Retrieval-Augmented Generation (RAG), and intelligent automation workflows.

With a background in Full Stack Development and hands-on experience in AI engineering, I enjoy creating systems that combine large language models, automation, and intuitive user experiences.

Currently focused on building scalable AI products, agentic workflows, and automation systems that deliver real impact.

---

## 🚀 About Me

- 🤖 Generative AI Developer passionate about building practical AI solutions
- 🧠 Experienced in RAG systems, AI automation, and agentic workflows
- 💻 Full Stack development background with MERN stack knowledge
- ⚡ Interested in AI products, workflow automation, and intelligent systems
- 📚 Constant learner exploring advanced LLM applications
- 🎯 Actively seeking opportunities in Generative AI / AI Engineering roles

---

## 🛠 Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

### Generative AI
![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge)
![Hugging Face](https://img.shields.io/badge/HuggingFace-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![ChromaDB](https://img.shields.io/badge/ChromaDB-purple?style=for-the-badge)

**Core Skills**
- Retrieval-Augmented Generation (RAG)
- Agentic Workflows
- Prompt Engineering
- Vector Databases
- Semantic Search
- NLP Pipelines
- Contextual Memory Systems

### Automation & AI Workflows
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=for-the-badge&logo=n8n&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-005571?style=for-the-badge)

### Full Stack Development
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white)
![Express](https://img.shields.io/badge/Express-black?style=for-the-badge&logo=express&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-43853D?style=for-the-badge&logo=node.js&logoColor=white)

---

## 💼 Experience

### Generative AI Trainee
**QuantumCrafter Studio Pvt. Ltd.**  
*Sept 2025 – Feb 2026*

- Built production-grade AI applications using LLMs and Retrieval-Augmented Generation
- Developed a PDF chatbot with semantic retrieval and contextual question answering
- Designed vector database pipelines using ChromaDB
- Improved retrieval quality and optimized performance in constrained API environments
- Worked on practical real-world AI engineering workflows

---

### Full Stack Developer Trainee
**SkillStone (Grazitti Interactive)**  
*Jan 2025 – May 2025*

- Developed a real-time chat application using MERN stack
- Implemented WebSocket-based real-time communication
- Built scalable backend interaction systems
- Improved frontend responsiveness and user interaction flow

---

## 🚀 Featured Projects

### 📄 RAG-Based PDF Chatbot
An intelligent chatbot that allows users to interact with PDF documents using natural language.

**Features:**
- Context-aware document Q&A
- Semantic retrieval using vector embeddings
- Optimized token usage for cost efficiency
- ChatGPT-style conversational interface

**Tech Used:**
Python • LangChain • ChromaDB • Cohere • Streamlit

---

### 📈 AI Stock Market Signal Automation
An AI-powered automation workflow for generating real-time stock trading signals.

**Features:**
- Event-driven automation workflows
- Live financial data integration
- Conditional decision-making logic
- Automated signal generation

**Tech Used:**
n8n • REST APIs • Workflow Automation • Frontend Integration

---

## 🎓 Education

### Chandigarh Engineering College
**Bachelor of Technology (ECE)**  
2021 – 2025

---

## 📜 Certifications

- IBM SkillsBuild — Generative AI
- OneRoadmap — AI Engineer Certificate

---


## 🌐 Connect With Me

📧 **Email:** tejasgoyal003@gmail.com  
💼 **LinkedIn:** https://linkedin.com/in/tejas-goyal  
💻 **GitHub:** https://github.com/tejasgoyal

---


⭐ If you like my work, feel free to connect or collaborate.
